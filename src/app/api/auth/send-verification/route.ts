...
export async function POST(req: Request) {
  ...
  const verificationLink = https://aetherord.pages.dev/verify-email?token=;
  ...
  const htmlContent = \
    <div style="background-color: #0a0a0a; color: #ffffff; font-family: 'Inter', sans-serif; padding: 40px 20px; text-align: center;">
      <div style="max-width: 600px; margin: 0 auto; background: #1a1a1a; border-radius: 24px; padding: 40px; border: 1px solid #333;">
        ...
        <h1 style="font-size: 28px; font-weight: 500; margin: 0 0 10px 0;">Welcome to Aether, \!</h1>
        ...
        <a href="\" style="display: inline-block; background-color: #ffffff; color: #000000; padding: 14px 32px; border-radius: 9999px; text-decoration: none; font-weight: 600; font-size: 16px; transition: background 0.3s;">
          Verify Email
        </a>
  ...
  try {
    ...
    sendSmtpEmail.subject = "Welcome to Aether – Verify your email";
    sendSmtpEmail.htmlContent = htmlContent;
    sendSmtpEmail.to = [{ email }];
    sendSmtpEmail.sender = {
      email: 'aether.verify@outlook.com',
      name: 'Aether'
    };
